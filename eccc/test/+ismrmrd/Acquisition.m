classdef Acquisition < matlab.mixin.Copyable
    ...
    methods(Access = protected)
        function obj = copyElement(old)
            
            % Shallow copy
            obj = old;
            
            % Deep copy
            obj.head = old.head.copy();
            obj.traj = old.traj;
            obj.data = old.data;
        end
    end
        ...
        function obj = Acquisition(arg1, traj, data)
            switch nargin
                case 0
                    ...
                    % initialize to an empty acquisition
%                     extend(obj,1);
                      obj.head = ismrmrd.AcquisitionHeader();
        ...
        function acq = select(obj, range)
            ...
            acq.traj = obj.traj(range);
            acq.data = obj.data(range);
        end
        
        function set(obj, range, acq)
            % Set a range of acquisitions
            
            % set header
            obj.head.set(range, acq.head);
            
            % set data
            obj.traj(range) = acq.traj;
            obj.data(range) = acq.data;
        end
