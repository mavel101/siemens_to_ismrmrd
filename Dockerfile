...
RUN apt-get update && apt-get install -y git cmake g++ libhdf5-dev libxml2-dev libxslt1-dev libboost-dev libboost-program-options-dev libboost-system-dev libboost-filesystem-dev libboost-thread-dev libboost-timer-dev libboost-program-options-dev libpugixml-dev libfftw3-dev
...
RUN apt-get update && apt-get install -y --no-install-recommends libxslt1.1 libhdf5-dev libfftw3-dev libpugixml-dev && apt-get clean && rm -rf /var/lib/apt/lists/*
...
ENTRYPOINT ["/usr/local/bin/siemens_to_ismrmrd"]
